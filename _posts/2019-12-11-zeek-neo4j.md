...
In this article, we'll show you how to capture network data from Zeek and analyze it in Neo4j. The diagram shows how the various components are physically and logically connected.

![Zeek/Neo4j topology](../img/zeek-neo-topology.png) 

[Zeek](https://www.zeek.org/), formerly called Bro, is a popular open-source intrusion detection system (IDS). Zeek's protocol analyzers extract around 400 fields from 35 protocols.
...
Elastic is a very popular platform to analyze logs. By default, Zeek's timestamp format isn't recognised by Elastic. The following setting changed the timestamp format to Elastic-friendly epoch millis:

    redef Kafka::json_timestamps = JSON::TS_MILLIS;

Avro is a common data serialization format that has several advantages over JSON. By default, Zeek outputs some of the data with periods in the field names. This isn't compatible with Avro. The following property replaces periods with Avro-friendly underscores:

    redef Log::default_scope_sep = "_";

Here are a few of the more interesting topics:
...
Once the Zeek data is in Kafka, it's possible to analyze using a variety of technologies. At the time of writing, there are over 100 connectors that can write to all sorts of databases (including SQL, columnar, document, cache, timeseries, GPU), CDC tools, messaging systems, and file/storage technologies.

The graph database is a natural fit to analyze network data since a network is, in fact, a graph of nodes (hosts) and edges (connections).
...
We've added some color coding to help make the Cypher statement easier to understand. Each event, highlighted in pink, is passed to the Cypher statement. Attributes of the event can be accesses with a `.`. For example, `event.id_orig_h` is the originating IP address of that event. We assign properties from the event to the `origin` and `response` nodes, and then create a directional relationship between those nodes. In addition to defining the nodes/edges, we captured counts for each connection and a sum of the bytes sent from each origin.

Neo4j has a (large) number of graph algorithms to help make sense of the data. We downloaded the latest Neo4j graph algorithms jar from the [the Neo4j download center](https://neo4j.com/download-center/), copied it into the plugins folder, and added the following line to `neo4j.conf`:
...
The degree of centrality is a measure of the number of edges connected to a given node, and the Louvain community detection algorithm groups similar nodes together.

The Neo4j Browser works really well for small datasets (i.e. < 300 nodes). For larger graphs, it's necessary to visualize using something else. [Neovis.js](https://github.com/neo4j-contrib/neovis.js/) worked really well. The chart below shows the graph of connections to hosts on my local network:

![sink Cypher statement](../img/neo4j-local.png) 

    # show traffic that originates within the network
    MATCH (x)-[y:CONNECTED_TO]->(z)
    WHERE z.id starts with '10.0.1.'
    RETURN *
...
This worked really well in my local network. In high-throughput environment, we'd recommend that you pre-aggregate events before writing them to Neo4j. This would be simple to do in KSQL or Kafka Streams.
