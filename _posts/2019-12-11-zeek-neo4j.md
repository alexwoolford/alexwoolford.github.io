...
[Elastic](https://www.elastic.co/products/elasticsearch) is very popular destination for cybersecurity logs and, out of the box, Zeek's timestamp format isn't recognised by Elastic. The following setting changed the timestamp format to Elastic-friendly epoch millis:
...
The [graph database](https://en.wikipedia.org/wiki/Graph_database) is a natural fit to analyze network data since a network is, in fact, a graph of nodes (hosts) and edges (connections).
